...
function [Infarct_WT] = wall_thinning_MI(results_struct,thresh,plot);

%To run this function, load results from GUI into workspace and run: 
%[Infarct_WT] = wall_thinning_MI(results_struct, 0.5,1) in the command window
% the percentage of infarct based on the threshold value (i.e. 0.5) will
% be calculated for end diastole 

%put 1 in place of "plot" if you would like to plot 0 if not

%Hint, you can change the threshold value to see how the percent infarct
%changes
...
data_tpt = transpose(squeeze(results_struct.length_r(tpt2,:,:)));
...
caxis([0 1.1]); 
c.Ticks = [0 0.5 1];
c.TickLabels = {'0.0 mm','0.5 mm','1.0 mm'}; c.FontName = 'Arial'; 
